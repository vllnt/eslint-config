  ...
  for (const { subpath, name } of PRESETS) {
    ...
    it(`${subpath} default === ${name}`, async () => {
      ...
      assert.equal(
        mod.default,
        mod[name],
        `default export !== named export "${name}"`,
      );
    });
  }
});
    ...
    it(`${subpath} entries are objects`, async () => {
      ...
      for (const entry of mod[name]) {
        assert.equal(
          typeof entry,
          "object",
          `entry in "${name}" is not an object`,
        );
      ...
      for (const entry of mod[name]) {
        ...
        for (const [pluginName, plugin] of Object.entries(entry.plugins)) {
          assert.equal(
            typeof plugin,
            "object",
            `plugin "${pluginName}" is not an object`,
          );
          assert.ok(plugin !== null, `plugin "${pluginName}" is null`);
          assert.ok(
            plugin.rules || plugin.configs,
            `plugin "${pluginName}" has no rules or configs`,
          );
        }
      }
    });
...
describe("TypeScript 6/7 compatibility", () => {
  it("runs type-aware linting through the TypeScript 6 JavaScript API", async () => {
    const eslint = new ESLint({
      cwd: join(__dirname, ".."),
      overrideConfigFile: true,
      overrideConfig: (await import("../flat/base.js")).base,
    });
    const [result] = await eslint.lintFiles(["tests/fixtures/typed.ts"]);
    ...
    assert.ok(
      ...
      "type-aware no-floating-promises rule did not run",
    );
  });

  it("exposes TypeScript 7 as tsc and TypeScript 6 as tsc6", () => {
    const bin = join(__dirname, "..", "node_modules", ".bin");
    const tsc = execFileSync(join(bin, "tsc"), ["--version"], {
      encoding: "utf8",
    });
    const tsc6 = execFileSync(join(bin, "tsc6"), ["--version"], {
      encoding: "utf8",
    });

    assert.match(tsc, /Version 7\.0\.2/);
    assert.match(tsc6, /Version 6\.0\./);
  });
});
...
describe("hygiene: no leaked internal references", () => {
  ...
  it('no "private" in package.json', () => {
    ...
    assert.equal(
      pkg.private,
      undefined,
      'package.json still has "private" field',
    );
  });
});
